...
% NOTE: Currently, the [DEFINITIONS] section does not include TE and TR.
%       Under the Siemens interperter (v1.4.3) this result in a wrong
%       reporting(!) of the TE and TR in the UI and the DICOM(!). However,
%       the actual TE and TR are correct.
%       The TE and TR are missing becuase the UI on the Siemens scanner
%       checks whether the TE and TR are within "allowed" range, but the
%       ranges are each a single value, so generally the TE and TRs are not
%       allowed.
%       Once the interperter is fixed (or for a different vendor), you can
%       uncomment the lines in the code:
...
% Define RO rephaser. 
% (Same as prepahser! the sum of prephaser and rephaser should cancel the
%  RO so they are both half the RO with reversed sign.)
...
for TRCounter = (-NumDummyScans+1):size(PE3DOrder, 1)
  ...
  eRO.amplitude  = ROAmp0 ;

  % Reset label used to mark if we should reverse the ADC.
  % (In bi-polar multi-echo, each time we flip the sign of the RO
  %  gradient). 
  ...
  for TECounter = 1:NumTEs
    ...
    if (TECounter > 1)
      if (Actual.bBipolarROGrads)
        ...
        if (mod(TECounter, 2) == 1) % odd (count from 1)
    ...
    if (TEFill < 0)
      error(['Cannot achieve desired TE[%d] = %f ms. ' ...
            ...
            TECounter, 1e3*Actual.TE(TECounter), ...
            1e3*(Actual.TE(TECounter) - TEFill))
    end
  ...
  if (Actual.bBipolarROGrads) % bi-polar RO gradients (alternating sign)
    eRephaseRO.amplitude =  (-1)^(NumTEs+1) * ePrephaseRO.amplitude ; % [Hz/m]
  else % mono-polar
