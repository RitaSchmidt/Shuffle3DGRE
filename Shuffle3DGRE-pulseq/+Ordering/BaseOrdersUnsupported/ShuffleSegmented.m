...
%   Length1 x Length2 
% Treats matrix as a 1D long ordered series, randomly shuffles the order in
% consecutive segments of length ~ShuffleDiameter and then does it again
% for shifted segments (so each new segment overlaps two of the previous
% segments).
...
%
% Shuffling is peformed twice. First the ordered 1D vector is split into
% consecutive segment (of length ~ShuffleDiameter) and each one is shuffled
% randomly. The process is the repeated for segments which are shifted
% (approximately) by one half a segment length.
%  
% Inputs:
% -------
...
%   [Default: Length1]
% - ShuffleDiameter - The (approximate) length of segments in whihc the
...
%
% NOTE: It seems the distrtibution of shifts between ordered and shuffled
%   positions is not uniformly distributed. Might need a better/different
%   algorithm.
%
% To Do:
% - Options to shuffle 2D locally.
% - Better 1D shuffling algorithm.
  ...
  % shuffle again, but consecutive "blocks" of size ~ShuffleDiameter which
  % overlap pairs of previuos "blocks"
  %
  % NOTE: might be better to combine with previous, where we exactly know
  %       which blocks we initially use.
  Idx = round(NIn / NSegs1) ;
  NEff = NIn - Idx ;
  NSegs2 = NSegs1 - 1 ;
  CountFromZeroCorr = 0 ;
  if (bCountFromZero)
  ...
  for Counter2 = 1:NSegs2
    Remaining = NEff - Idx ;
    RangeLength = round(Remaining / (NSegs2 - Counter2 + 1)) ;
    Range = Idx + (1:RangeLength) ;
    IdxPerPosOut(Range) = Range(randperm(RangeLength)) + CountFromZeroCorr ;
    Idx = Range(end) ;
  end
