function [OrderMat,IOut,JOut,MaskOrderOut] = ...
                           LocalShuffleInputTrajectory(OrderMatIn, ...
                                                       TRtimesCutoffFreq, ...
                                                       bMask, RandomSeed, ...
                                                       bPlot)
...
% subsampling mask is first applied and then Local Shuffling of the order
% is performed.
%
% Inputs:
% -------
...
%% Initial setup - set defaults for missing inputs
  ...
  elseif(TRtimesCutoffFreq < 0)
    ...
    % Internally, ShuffleLocal round ShuffleDiameter, so we round it here
    % as well.
    ShuffleDiameter = round(1/(1.2*TRtimesCutoffFreq)) ; % x1.2 is emprirical

    % Shuffle a 1D vector of the same length as (the new) IOut and Jout.
    [~, IJReorder] = ...
                Ordering.Utils.ShuffleLocal(numel(IOut), 1, ...
                                            ShuffleDiameter, ...
                                            RandomSeed,  ...
                                            false, ... % not meandering
                                            false, ... % Count from 1
                                            false, ... % no plotting
                                            []) ; % No SwapDistsWeightsIn
  end
  ...
  if (bPlot)
    TitleStr = sprintf('Shuffle Local %dx%d ("shuffle diameter" = %g)', ...
